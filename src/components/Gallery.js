...
import { fetch_art, fetch_comments, fetch_count } from "../reducer/Art/artActions";
...
function Gallery() {
  ...
  function ShowModal(x) {
    ...
    dispatch(fetch_comments(x.artModelId))
  }
  function CloseModal() {
  ...
  const gallery = results.data.map((x) => (
    <div  key= {x.artModelId} className=" col-md-4 col-sm-6 col-xl-3 d-flex ">
      <Card className=" card-artwork m-2 imageCard ">
        ...
        <Card.Body>
          ...
          <Card.Text>
            Created by {x.artist} and installed in the year {x.yeaR_INSTALLED}.
           {PreviewText(x.description)}
            <div className="comments mt-2 d-flex justify-content-end">
              <p className="mr-2">Comments </p>
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" className="bi bi-chat-quote" viewBox="0 0 16 16">
